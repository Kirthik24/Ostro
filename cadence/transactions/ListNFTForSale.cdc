...
transaction {
    prepare(acct: AuthAccount) {
        let receiver = acct.getCapability<&{OstroToken.Receiver}>(/public/MainReceiver)
        let sale <- MarketplaceContract.createSaleCollection(ownerVault: receiver)

        let collectionRef = acct.borrow<&PropertyContract.Collection>(from: /storage/NFTCollection)
